...
def sampleVolume(train_loader, contrast_value=4000):
    # Sample a batch of data from the dataloader
    batch = next(iter(train_loader))

    # Separate the image and segmentation from the batch
    volumes, target = batch["image"], batch["seg"]
    volume = add_vessel_contrast(volumes, target, contrast_value=contrast_value)

    return volume, target


def trainSingleEpoch(
    model, criterion, optimizer, Volume, Target, device, length, verbose=False
):
...
def testModel(model, criterion, Volume, Target, device):
    ...
    for i in range(5):
        ...
        drr_vessels = ((drr_vessels / (torch.max(drr_vessels.flatten()))) * 2).to(
            device
        )
...
def trainModel(epochs, length, save, load, path, verbose=False):
    ...
    for i in range(epochs):
        trainSingleEpoch(
            model, criterion, optimizer, volume, target, device, length, verbose
        )
...
def testInferenceSpeed(path):
    ...
    drr_input, drr_vessels = drr_to_input(
        drr_raw, enhancement_factor=0.6, device=device
    )
...
def visualizeModel(model, Volume, Target, device):
    ...
    drr_input, drr_vessels = drr_to_input(
        drr_raw, enhancement_factor=0.5, device=device
    )
...
def visualizationResult(model, Volume, Target, device):
    ...
    drr_input, drr_vessels = drr_to_input(
        drr_raw, enhancement_factor=0.4, device=device
    )
    prediction, latent_representation = model(Target.to(device), drr_input)
    AI_enhanced = drr_body + prediction
    ...
    drr_input, drr_vessels = drr_to_input(
        drr_raw, enhancement_factor=0.1, device=device
    )
    prediction, latent_representation = model(Target.to(device), drr_input)
    AI_enhanced = drr_body + prediction
...
def rotationSweep(model, Volume, Target, device, criterion):
    ...
    for j in range(10):
        ...
        for i in range(80):
            ...
            drr_input, drr_vessels = drr_to_input(
                drr_raw, enhancement_factor=ef, device=device
            )
            prediction, latent_representation = model(Target.to(device), drr_input)
